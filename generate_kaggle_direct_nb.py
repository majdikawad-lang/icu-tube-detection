 ...
 "cells": [
  {
   ...
   "metadata": {},
   "source": [
    "# 🩺 التدريب المباشر على خوادم Kaggle\n",
    "هنا لا نحتاج لتحميل البيانات لأننا داخل موقع Kaggle والبيانات موجودة بالفعل!"
   ]
  },
  {
   "cell_type": "code",
  ...
  },
  {
   ...
   "outputs": [],
   "source": [
    "# -- تجهيز البيانات باستخدام مسارات Kaggle المباشرة --\n",
    "class RANZCRDataset(Dataset):\n",
    "    def __init__(self, csv_file, img_dir):\n",
    "        self.df = pd.read_csv(csv_file)\n",
    "        # الفلترة لتسريع التدريب كبداية\n",
    ...
    "\n",
    "print('تجهيز البيانات من مسار مسابقة Kaggle...')\n",
    "CSV_PATH = '/kaggle/input/ranzcr-clip-catheter-line-classification/train_annotations.csv'\n",
    "IMG_DIR = '/kaggle/input/ranzcr-clip-catheter-line-classification/train'\n",
    "\n",
    "dataset = RANZCRDataset(CSV_PATH, IMG_DIR)\n",
  ...
  },
  {
   ...
   "source": [
    ...
    "criterion = DiceLoss()\n",
    "\n",
    "EPOCHS = 10 # 10 دورات للحصول على أعلى دقة طبية ممكنة\n",
    "for epoch in range(EPOCHS):\n",
    "    model.train()\n",
    "    total_loss = 0\n",
    "    for i, (imgs, masks) in enumerate(loader):\n",
    "        if imgs.sum() == 0: continue\n",
    ...
    "\n",
    "# حفظ الموديل في مسار المخرجات المتاح للتحميل في Kaggle\n",
    "torch.save(model.state_dict(), '/kaggle/working/best_unet_model.pth')\n",
    "print('✅ انتهى التدريب الحقيقي! قم بتحميل best_unet_model.pth من قسم Output على اليمين.')"
   ]
  }
 ],
 "metadata": {},
...
with open('ICU_Kaggle_Direct.ipynb', 'w', encoding='utf-8') as f:
